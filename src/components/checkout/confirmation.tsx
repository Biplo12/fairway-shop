...
/**
 * The receipt. The bag is emptied here rather than at the checkout, so a
 * customer who backs out halfway still has their clubs, and the order is
 * photographed first: once the bag is cleared there is nothing left to read.
 *
 * Reloading this page shows the reference without the lines, which is honest.
 * There is no order behind it to fetch.
 */
export function Confirmation({
  ...
  // After the paint, never during render: the same shape the bag itself uses to
  // read its store, and the order is copied before the bag is emptied.
  useEffect(() => {
      ...
      <div className="mt-12 grid gap-12 lg:grid-cols-[1fr_1fr] lg:gap-16">
        ...
        <div className="rounded-card bg-paper p-6 md:p-8">
          ...
          {placed ? (
            <>
              <ul className="mt-6">
                {placed.lines.map(({ product, quantity }) => (
                  <li
                    key={product.slug}
