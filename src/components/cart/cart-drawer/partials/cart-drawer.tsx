  ...
  return (
    ...
    >
      <button
        ...
        className={cn("absolute inset-0 bg-charcoal/45 transition-opacity duration-300", open ? "opacity-100" : "opacity-0")}
      />
      ...
      <div
        ...
        aria-label="Your bag"
        tabIndex={-1}
        className={cn("absolute inset-y-0 right-0 flex w-full max-w-[30rem] flex-col bg-offwhite transition-transform duration-300 ease-out focus:outline-none", open ? "translate-x-0" : "translate-x-full")}
      >
        ...
        {lines.length === 0 ? (
          ...
          <ul className="flex-1 overflow-y-auto px-5 md:px-7">
            {lines.map(({ key, product, quantity, options, unitPrice }) => (
              <li
                key={key}
